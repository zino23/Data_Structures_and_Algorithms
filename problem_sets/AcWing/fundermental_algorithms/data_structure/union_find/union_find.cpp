...
const int N = 1e5 + 10;
int n, m;
int p[N]; // simulate a tree with an array which stores a collection of
          // disjoint-sets
...
int main() {
  ...
  cin >> n >> m;
  for (int i = 1; i <= n; i++) {
    p[i] = i; // initialize p disjoint-sets, with node indexes from 1 to n whose
              // ancestor is their index
  }
  ...
  while (m--) {
    ...
    if (*op == 'M')
      p[find(a)] =
          find(b); // let b's ancestor be a's ancestor, i.e. merge a into b
    else {
      if (find(a) == find(b))
        puts("Yes");
      else
